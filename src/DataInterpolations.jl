...
Return the number of dimensions `ndims(x(t))` of interpolation `x` evaluated at a single value `t`
...
Return the size `size(x(t))` of interpolation `x` evaluated at a single value `t`.
"""
