...
## Arguments
  ...
  - `knotVecType`: symbol to knot vector, `:Uniform` for uniform knot vector, `:Average` for average spaced knot vector.
    `:Average` is the appropriate choice for interpolation. `:Uniform` places the interior knots on a grid that is
    inconsistent with the parameter spacing, which makes the collocation system exponentially ill-conditioned in the
    number of points for `d >= 3` (and for `pVecType = :ArcLen` at any degree); constructing such an interpolation
    warns, and the result may deviate from the data by orders of magnitude between the data points.
...
function BSplineInterpolation(
    ...
    if knotVecType == :Uniform
        # Uniformly spaced knot vector. The interior knots are spaced `1/(n - d)` while
        # the parameters are spaced `1/(n - 1)`, so the two grids drift apart by up to
        # `(d - 1)/2` knot spans across the domain. Past half a span the collocation
        # system's conditioning degrades exponentially in `n`, which makes this choice
        # unusable for `d >= 3`, and for the chord length method at any degree (#567).
        for i in (d + 2):n
    ...
    F = bspline_collocation_factorization(sc, n, d, pVecType, knotVecType)
    c = vec(F \ u[:, :])
    ct = F \ collect(t)
    sc = zeros(eltype(t), n)
